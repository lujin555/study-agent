...
> **关于生成 100% 的诚实说明**：CS-Notes 是通用技术知识，模型本身就可能知道这些答案，因此生成正确率**无法证明 RAG 检索的增量贡献**。严谨的做法是消融实验：同一批题分别走"有检索"和"纯模型"两条链路对比，量化 RAG 带来的提升（见 Roadmap）。当前 100% 只说明"链路正确、不添乱"，不代表"检索已足够好"。
