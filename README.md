...
| **多格式资料** | 支持 PDF / DOCX / TXT / Markdown，支持拖拽上传与目录监听自动入库 |
| **扫描版 PDF OCR** | 无文字层的影印版 PDF 自动走 RapidOCR 识别后入库（`OCR_ENABLED=true` 开启，默认关闭），识别内容同样参与检索与答案溯源 |
...
flowchart TB
    subgraph ingest["资料入库（离线）"]
        D1["docs/ 文档<br/>PDF · DOCX · TXT · MD"] --> D2{"有文字层？"}
        D2 -->|"有"| D3["文本提取<br/>PyMuPDF / python-docx"]
        D2 -->|"无 · 扫描版"| OCR["RapidOCR 识别<br/>取内嵌图原分辨率"]
        D3 --> D4["递归切块<br/>500 字 / 50 重叠"]
        OCR --> D4
        D4 --> D5["bge-small-zh-v1.5<br/>本地向量化"]
        D5 --> DB[("ChromaDB<br/>向量库")]
    end
...
**数据流向**：文档 → 判断有无文字层（无则走 RapidOCR 识别内嵌图）→ 文本提取 → 切块 → 向量化 → 存入 ChromaDB；提问时 Agent 判断是否需要检索 → 向量召回与 BM25 关键词召回**双路并行**，经 RRF 融合取 Top-K，再按距离阈值过滤 → 片段拼进上下文 → 大模型基于片段作答 → 流式吐回前端。
...
| **RapidOCR + 内嵌图原分辨率** | 扫描件常把小图塞进超大画布（实测单页 8057×10316pt，A4 仅 595×842），渲染整页等于把 1080px 插值放大到 2 万+像素，既触发 FzErrorLimit 又因模糊拉低识别率；直接取 PDF 内嵌图才是原始清晰度。识别结果用「置信度 + 行长度」双重过滤：置信度挡"没看清"的错字，长度挡"看清了但没意义"的图表碎片 |
...
影印版教材、试卷没有文字层，`PyMuPDF` 提取不到任何文本（旧版本会直接报"扫描版 PDF"错误）。开启 OCR 后（`.env` 设 `OCR_ENABLED=true`，默认关闭），这类 PDF 自动走识别链路：逐页取出**内嵌图片**（而非渲染整页）→ RapidOCR 识别 → 按「置信度 ≥ `OCR_MIN_SCORE` 且行长 ≥ `OCR_MIN_LINE_LEN`」双重过滤 → 与正常文本一样切块、向量化入库。

实测（1 页数据库试卷，含 E-R 图与手写答案区）：
...
入库后扫描件与普通资料完全同权：可检索、可问答、可溯源。

![扫描版试卷 OCR 入库后的问答与溯源](assets/ocr-scan-qa.png)

> 截图里展开的 Agent 过程面板可见：`search_notes` 检索回来的片段正是 OCR 识别出的试卷原文，回答末尾的 `[来源：计组和数据库.pdf]` 是自动溯源标注。

> **已知局限**：图表与手写区会产出**高置信度碎片**（如"质工""秀S""主石"）。置信度只衡量"看清了吗"，不衡量"有意义吗"，所以置信度过滤挡不住它们，长度过滤仅能缓解；彻底解决需要 LLM 后处理清洗（见 Roadmap）。
...
添加资料：把文档拖进 `docs/`，运行 `python ingest.py` 入库；或 `python watch_docs.py` 监听目录自动入库。扫描件需先在 `.env` 设 `OCR_ENABLED=true`。
...
├── rag/
│   ├── loader.py       # 多格式文本提取 + 扫描版 OCR（取内嵌图原分辨率）
...
- **OCR 默认关闭**：需在 `.env` 设 `OCR_ENABLED=true` 才识别扫描件（OCR 约 2 秒/页，会显著拖慢 ingest）；图表/手写区的高置信度碎片仍会混入（见 OCR 章节）
...
- **令牌为无状态签名**：token = 时间戳 + HMAC 签名（迷你 JWT 思想），服务重启登录态不失效，24 小时自动过期

---

## Roadmap

- [ ] **OCR 碎片清洗**：用 LLM 后处理清洗图表/手写区产出的高置信度碎片（如"质工""秀S"），提升扫描件入库质量
...
Python · FastAPI · Vue 3 · Vite · ChromaDB · sentence-transformers · RapidOCR · SQLite · Docker · DeepSeek API
